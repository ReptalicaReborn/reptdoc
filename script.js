...
function applyCustomAccent(hexColor) {
    // Convert hex to rgb for mixColors compatibility
    const r = parseInt(hexColor.slice(1, 3), 16);
    const g = parseInt(hexColor.slice(3, 5), 16);
    const b = parseInt(hexColor.slice(5, 7), 16);
    ...
    // Dark Mode overrides (Using neutral base for pure color tint)
    const mixBase = '#050505'; // Neutral dark base instead of warm brown

    root.style.setProperty('--md-sys-color-primary', rgbColor);
    root.style.setProperty('--md-sys-color-on-primary', contrastColor); // Dynamic contrast
    ...
    // Tint the surface container (cards/sidebar) - Increased visibility
    ...
    // Tint Secondary Container (for active nav items)
    ...
    // Tint the main background and surface very subtly
    ...
    // Update meta theme color to match the tinted background
    const metaThemeColor = document.querySelector('meta[name="theme-color"]:not([media])');
    if (metaThemeColor && !document.body.classList.contains('light-mode')) {
    ...
    const lightOnPrimary = contrastColor; // Dynamic contrast
    ...
    container.innerHTML = `
        <div class="welcome-container">
            ...
            <div class="welcome-features">
                ...
                <div class="welcome-card" onclick="document.getElementById('theme-toggle').click()">
                    <div class="welcome-card-content">
                        <span class="material-icons-round welcome-card-icon">dark_mode</span>
                        <h3>${t('card_theme_title')}</h3>
                        <p>${t('card_theme_desc')}</p>
                    </div>
                </div>
            </div>
        </div>
...
function mixColors(color1, percent, color2) {
    // Parse color1 (expected as rgb(r, g, b))
    ...
    // Parse color2 (expected as #RRGGBB)
...
function initTheme() {
    ...
    // Helper to sync meta theme color with site theme (Static colors only)
    const updateMetaColor = (isLight) => {
        // Find the generic meta tag first, or media tags
        let meta = document.querySelector('meta[name="theme-color"]:not([media])');
        if (!meta) {
            // Remove existing media tags to avoid conflict if we are manually overriding
            document.querySelectorAll('meta[name="theme-color"]').forEach(m => m.remove());
            meta = document.createElement('meta');
            meta.name = 'theme-color';
            document.head.appendChild(meta);
        }
        // Force the color to match the site theme background
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check for saved custom accent color
    const isAccentEnabled = localStorage.getItem('reptdoc_accent_enabled') === 'true';
